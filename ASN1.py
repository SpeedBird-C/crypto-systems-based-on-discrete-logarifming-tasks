...
def packELsignASN1(w, s, b, p, r, a, str_id):
    str_id = ASN1packstr(str_id) # строка со словами подпись эль-гамаль
    w = ASN1packint(w) # часть подписи
    s = ASN1packint(s) #другая часть подписи
    b = ASN1packint(b) # одна из тройки открытого ключа, ОТКРЫТЙ КЛЮЧ
    p = ASN1packint(p) # простая харктеристика поля G, вторая из тройки открытого ключа
    r = ASN1packint(r) # порядок группы r
    a = ASN1packint(a) # образующая a, генератор , часть открытого ключа
